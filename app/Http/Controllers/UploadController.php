...
class UploadController extends Controller
{
    ...
    {
        return $path = $request->file('avatar')->storePublicly('avatars', 's3');
        ...
        if($request->has('file') && strpos($request->file, ';base64')){
            ...
            //envia o arquivo
            $data = Storage::put($path.$name, base64_decode($file));

            return response()
            ->json(['content' => ['file' => $name], 'data' =>  $data, 'Message' => 'Arquivo enviado com sucesso'], 201);
